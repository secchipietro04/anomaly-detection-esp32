...
static esp_err_t ism330bx_read_reg_spi(ism330bx_spi_dev_t *dev, uint8_t reg_addr, uint8_t *data, size_t len) {
  ...
  if (!tx_buf || !rx_buf) {
    if (tx_buf && tx_buf != stack_tx) free(tx_buf);
    if (rx_buf && rx_buf != stack_rx) free(rx_buf);
  ...
  if (tx_buf != stack_tx) free(tx_buf);
  if (rx_buf != stack_rx) free(rx_buf);
...
static esp_err_t ism330bx_fetch_fifo_buffer(ism330bx_spi_dev_t *dev, ism330bx_fifo_result_t *result) {
  ...
  uint8_t status1 = 0;
  uint8_t status2 = 0;
  esp_err_t ret = dev->read_reg(dev, ISM330BX_REG_FIFO_STATUS1, &status1, 1);
  if (ret != ESP_OK) return ret;
  ret = dev->read_reg(dev, ISM330BX_REG_FIFO_STATUS2, &status2, 1);
  if (ret != ESP_OK) return ret;

  uint16_t fifo_samples = status1 | ((status2 & ISM330BX_FIFO_STAT2_DIFF_MASK) << 8);
  ...
  uint16_t samples_to_read = fifo_samples;
  if (samples_to_read > result->accel_data_size) {
    samples_to_read = result->accel_data_size;
  }
...
static esp_err_t ism330bx_apply_config(ism330bx_spi_dev_t *dev, const ism330bx_config_t *cfg) {
  ...
  // 1. CTRL3_C (0x12): BDU = 1 (bit 6), IF_INC = 1 (bit 2) -> 0x44
  uint8_t ctrl3_c = 0x44;
  dev->write_reg(dev, ISM330BX_REG_CTRL3_C, &ctrl3_c, 1);

  // 2. CTRL1_XL (0x10): ODR_XL in bits [3:0], OP_MODE_XL = 000 (High Performance) in bits [6:4]
  uint8_t ctrl1_xl = cfg->accel_odr & 0x0F;
  esp_err_t ret = dev->write_reg(dev, ISM330BX_REG_CTRL1_XL, &ctrl1_xl, 1);
  if (ret != ESP_OK) return ret;

  // 3. CTRL2_G (0x11): ODR_G in bits [3:0], OP_MODE_G = 000 (High Performance) in bits [6:4]
  uint8_t ctrl2_g = cfg->gyro_odr & 0x0F;
  ret = dev->write_reg(dev, ISM330BX_REG_CTRL2_G, &ctrl2_g, 1);
  if (ret != ESP_OK) return ret;

  // 4. CTRL8_XL (0x17): FS_XL in bits [1:0]
  uint8_t ctrl8_xl = cfg->accel_fs & 0x03;
  dev->write_reg(dev, ISM330BX_REG_CTRL8_XL, &ctrl8_xl, 1);

  // 5. CTRL6_G (0x15): FS_G in bits [3:0]
  uint8_t ctrl6_g = cfg->gyro_fs & 0x0F;
  dev->write_reg(dev, ISM330BX_REG_CTRL6_G, &ctrl6_g, 1);

  // 6. FIFO_CTRL3 (0x09): BDR_GY in bits [7:4], BDR_XL in bits [3:0]
  uint8_t fifo_ctrl3 = ((cfg->fifo_bdr_gy & 0x0F) << 4) | (cfg->fifo_bdr_xl & 0x0F);
  ret = dev->write_reg(dev, ISM330BX_REG_FIFO_CTRL3, &fifo_ctrl3, 1);
  if (ret != ESP_OK) return ret;

  // 7. Bypass FIFO first to clear (FIFO_CTRL4 = 0x0A)
  uint8_t fifo_bypass = 0x00;
  dev->write_reg(dev, ISM330BX_REG_FIFO_CTRL4, &fifo_bypass, 1);

  // 8. Set FIFO operating mode (FIFO_CTRL4 = 0x0A, 0x06 = Stream Mode)
  uint8_t fifo_ctrl4 = cfg->fifo_mode & 0x07;
  ret = dev->write_reg(dev, ISM330BX_REG_FIFO_CTRL4, &fifo_ctrl4, 1);

  ESP_LOGI(TAG, "Configured sensor: CTRL1_XL=0x%02X, CTRL2_G=0x%02X, FIFO_CTRL3=0x%02X, FIFO_CTRL4=0x%02X",
           ctrl1_xl, ctrl2_g, fifo_ctrl3, fifo_ctrl4);
...
ism330bx_odr_t ism330bx_hz_to_odr(float hz) {
    if (hz <= 1.875f)  return ISM330BX_ODR_1u875Hz;
    if (hz <= 7.5f)    return ISM330BX_ODR_7u5Hz;
