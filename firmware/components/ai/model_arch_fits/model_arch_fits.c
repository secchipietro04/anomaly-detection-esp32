...
static const char* TAG = "MODEL_FITS";

static bool fits_interface_run(anomaly_model_t* model, const float* input_data, size_t input_len, const fft_shared_data_t* shared_fft, float* out_anomaly_score) {
    if (!model || !model->instance_data || !input_data || !out_anomaly_score) return false;
    model_arch_fits_impl_t* impl = (model_arch_fits_impl_t*)model->instance_data;
    ...
    if (required_bytes > in_det->bytes) {
        ESP_LOGE(TAG, "STFT output size (%zu) exceeds FITS input tensor capacity (%zu)", required_bytes, in_det->bytes);
        return false;
    }

    // Zero-copy sharing: check if we can reuse the computed STFT spectrogram
...
static void fits_interface_deinit(anomaly_model_t* model) {
...
static bool fits_interface_update(anomaly_model_t* model, const uint8_t* new_model_bytes) {
...
bool model_arch_fits_anomaly_init(anomaly_model_t* model, const uint8_t* model_bytes, const base_model_config_t* config, size_t window_size, size_t hop_size, float gamma) {
    ...
    model_arch_fits_impl_t* impl = (model_arch_fits_impl_t*)calloc(1, sizeof(model_arch_fits_impl_t));
    ...
    model->name = "FITS Model (ft mode)";
    model->model_id = (uint32_t)(uintptr_t)impl;
    model->type_id = ModelType_model_arch_FITS_m_c;
    model->instance_data = impl;
    model->run = fits_interface_run;
    model->deinit = fits_interface_deinit;
    model->update = fits_interface_update;
