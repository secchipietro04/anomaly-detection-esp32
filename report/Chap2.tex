...
Il sistema è stato sviluppato con un'architettura distribuita di tipo \textit{edge-to-cloud}, progettata per disaccoppiare le fasi di acquisizione dati vibrazionali, l'inferenza di modelli \textit{on edge} per il rilevamento delle anomalie e l'addestramento di questi modelli.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Ha una latenza più bassa rispetto a MQTT nel trasferimento di grandi quantità di dati.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Necessario Broker:} necessita di un broker che gestisca la comunicazione tra gli endpoint.
		\item \textbf{Latenza maggiore:} l'intermediazione del broker e i meccanismi di QoS, soprattutto di livello 1 o 2 introducono ritardi rispetto a comunicazioni dirette.

	\end{itemize}
\end{itemize}

Per la leggerezza del protocollo e per il sistema a \textit{publish/subscribe} è stato scelto MQTT per tutte le comunicazioni tra l'edge device e il servizio.
La parte più critica è la trasmissione dei modelli, essendo di dimensione non trascurabile, per questo tipo di comunicazione si sarebbe potuta usare un'architettura ibrida dove via MQTT l'edge device viene notificato del caricamento dei nuovi modelli che verrebbero poi scaricati via una richiesta GET di HTTP usando le funzioni di \textit{chunking} del payload.

\section{Scelta del Protocollo di Presentazione Dati}
\label{sec:scelta_cbor}
È necessaria una Lingua Franca per la rappresentazione dei dati condivisa che permetta ai vari dispositivi nella rete di codificare e decodificare i dati in un formato condiviso, come descrive il livello 6 del modello \textit{ISO/OSI} \cite{isoosi}.
Le scelte si dividono in protocolli basati su testo (XML \cite{bray2008xml}, JSON \cite{bray2017json}) e protocolli a rappresentazione binaria (CBOR \cite{bormann2020cbor}, BSON \cite{bson2009spec}, Protocol Buffers \cite{varda2008protobuf}, MessagePack \cite{furuhashi2013msgpack}, Avro \cite{apache2011avro}).
...
Il JSON risulta notevolmente più leggero (30-40\% di dimensione in meno \cite{cbor_comp} ), umanalmente più leggibile e rapido da elaborare via codice rispetto alla prolissità e alla complessità strutturale dell'XML.
...
Data la tabella la migliore scelta è \textit{CBOR} (Concise Binary Object Representation), per i seguenti motivi: è un protocollo \textit{royalty-free} e \textit{open source}, è stato sviluppato dalla base per essere usato in contesti IoT per usare poca memoria e supporta nativamente la criptazione dei dati, attraverso lo standard COSE (CBOR Object Signing and Encryption, RFC 9052 \cite{schaad2022cose}).


Viene usata come libreria sul microcontrollore \textit{espressif/cbor} \cite{espressif_cbor}, derivata da \textit{tinycbor} \cite{tinycbor}, ed è impiegato anche \textit{zcbor} \cite{zcbor}, un programma che permette di tradurre lo schema CDDL \cite{birkholz2019cddl} direttamente in codice C ottimizzato per la codifica e/o decodifica dei pacchetti in struct.

\begin{samepage}
	
\section{Architettura backend}
Il backend è formato da servizi ospitati su container Docker \cite{merkel2014docker}, il che permette di rendere il servizio modulare e facilmente riproducibile.
I servizi docker sono:

\begin{itemize}

	\item \textbf{emqx}: \cite{emqx} Un broker MQTT, versione Community, gestisce la connessione MQTT.
	\item \textbf{postgres}: \cite{postgresql} un database relazionale con estensione TimescaleDB \cite{timescaledb}, che migliora le prestazioni della base di dati nella gestione, scrittura e analisi di dati temporali.
	\item \textbf{api\_service}: Server FastAPI \cite{fastapi} per la gestione dei dati del database.
	\item \textbf{mqtt\_service}: Si interfaccia al servizio MQTT, ricevendo i dati dal sensore e pubblicandogli i modelli e configurazioni, gestendone il lifecycle.
	\item \textbf{nas\_service}: Neural Architecture Search, Servizio che usa Optuna \cite{optuna2019} per la ricerca architetturale e allenamento modelli tflite \cite{david2021tflitemicro} specifici al sensore.
	\item \textbf{grafana}: \cite{grafana} Dashboard per visualizzazione dei dati e risultati di inferenza.
	\item \textbf{clearml\_*}: \cite{clearml} Piattaforma per tracciare parametri, loss e metriche dei modelli, divisa in 6 container:
	\begin{itemize}
		\item API
		\item Web
		\item Files
...
Come sensore MEMS viene usato il modulo \textit{STEVAL-MKI245KA} \cite{mems_board}, di STMicroelectronics, un kit di valutazione del circuito integrato \textit{ISM330BX} \cite{ism330bx_datasheet}, concesso in uso per le attività di ricerca da Iotinga SRL.
...
Come \textit{framework} di sviluppo viene usato ESP-IDF \cite{espressif_espidf}, scelto per il controllo più a basso livello, rispetto ad Arduino Core, delle periferiche hardware e la gestione nativa dei task FreeRTOS.
